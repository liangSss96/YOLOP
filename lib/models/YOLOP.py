...
class MCnet(nn.Module):
    def __init__(self, block_cfg, **kwargs):
        ...
        for i, (from_, block, args) in enumerate(block_cfg[1:]):
            ...
            if block is Detect:
                args[0] = cfg.DATASET.CLASS_NUMBER
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b.data[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image) 这部分调整主要是为了匹配每个图像中大约有8个目标的预期数量。
            # 这部分调整主要是为了平衡不同类别之间的样本不平衡问题，以提高模型对少见类别的检测能力。
...
if __name__ == "__main__":
    ...
    model = get_net(False)
    print("model.detector_index", model.detector_index)
